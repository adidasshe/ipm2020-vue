...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        username: [
          { required: true, message: "用户名必须填写", trigger: "change" }
        ]
      }
    };
  },
  ...
  methods: {
    ...
    updateData() {
      this.$refs["adminForm"].validate(valid => {
        if (valid) {
          updateUser(this.admin.id, this.admin).then(() => {
            this.getList();
            this.dialogVisible = false;
            this.$message({
              message: "添加成功！",
    ...
    getRoleListByAdmin(adminId) {
      getRoleByUser(adminId).then(response => {
